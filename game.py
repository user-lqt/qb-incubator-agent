...
ENDINGS = {
    ...
    "E_TIMELINE": {
        ...
        "tagline": "记录失败。时间线重置。",
        "closing": (
            "本轮进入结局【第十二次轮回】。请以 QB 的身份收束：对局已经结束，"
...
# 序幕：玩家视角的背景设定。**只用主角知道的信息**——
# 这里出现的每一条都是"一个人刚开学时会知道的事"，绝不提前交代世界观、身份与代价。
OPENING = """【序幕 · 走廊尽头】
...
EPILOGUES = {
    ...
    "E_OTHER": (
        ...
        "开头几年不算轻松，可你守住了自己的节奏：学会下班后关掉消息，"
        "学会把体检报告从头看完，学会当面说这个方案不合理。\n"
        "第十年，风向确实换了一次，同行倒了一片。你没有飞起来，也没有掉下去——"
        "你只是还在做那件事，而且比谁都熟。\n"
        "后来你写的那套小系统被三个工地把合同续了三年；不是丰碑，"
        "可能只是别人嫌麻烦、你顺手做了的那一块，但每天真的有人在用。\n"
        "你的名字没被印在重点工程名单的第十一页，它写进了同事的通讯录、"
        "写进了几个真心记得你的人那里，也写进了家里那盏一直等你的灯。\n"
        "某个傍晚你路过一片工地，塔吊转过来，你忽然明白它当年算漏了一件事："
        "它把人的一生当成一次押注，而你本来就可以慢慢来。\n"
        "它偶尔还会在走廊上停一下，礼貌地问一句。你说不了。它点点头，在记录里写："
...
def epilogue(ending: str) -> str:
    return EPILOGUES.get(ending, "")
...
def chat(question: str, history=None, state=None, intent: str = "") -> dict:
    ...
    return {
        ...
        "epilogue": epilogue(ending) if ending else "",
        "ending": ending,
