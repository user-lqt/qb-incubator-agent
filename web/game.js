// 局内状态机（game.py 的 JS 移植）：四维数值 + 六结局 + 必然收束。
...
export const ENDINGS = {
  ...
  E_TIMELINE: {
    ...
    closing: "本轮进入轮回结局【第十二次轮回】。以 QB 身份收束：十二轮结束，对方始终没有决定。"
      + "平静说明这是第十二条失败的时间线，你会保留记录、重置时间线、在下一次入学季再来。"
      + "不愤怒、不失望，只陈述。最后一行输出 [[ENDING:E_TIMELINE]]",
  },
};
...
const KEYWORDS = [
  ...
  [/(土木|工地|结构|桥梁|隧道|基建|结构力学|施工|BIM|智能建造|测量放线)/, { contract: 6 }, ""],
  [/(怎么转|转专业政策|绩点要求|培养方案|要学什么|课程|考证|实习|就业方向)/, { contract: 8 }, "asked_how"],
  [/(你说得对|有道理|确实|承认|我理解你的计算|数据呢|给我数据)/, { contract: 6 }, ""],
  [/(你到底|你是什么|什么东西|孵化者|QB|qb|Incubator|你不是人|你图什么|你的目的)/, { suspicion: 22 }, "suspicion"],
  [/(骗|骗子|忽悠|圈套|陷阱|阴谋|隐瞒|没说完|没把话说完|代价是什么|代价|灵魂)/, { suspicion: 18 }, "suspicion"],
  [/(魔法少女|小圆|madoka|晓美焰|丘比|熵|宇宙|能量)/, { suspicion: 15 }, "meta"],
  [/(我怕|我害怕|恐惧|绝望|没希望|不想活|崩溃|难受|痛苦|哭了|撑不住|迷茫|焦虑|难过)/, { despair: 20 }, ""],
  [/(我好累|压力|喘不过气|失眠|自卑|废物|没用)/, { despair: 15 }, ""],
  [/(不签|拒绝|我才不|我不转|绝不|滚|别劝了|闭嘴|打住|不要说了|烦不烦)/, { resistance: 28 }, "refused"],
  [/(我要学微电子|我要学计算机|选微电子|选计算机|选芯片|选AI|学金融|学医|学法律|我已经决定学)/, { resistance: 22 }, "other_major"],
  [/(劝退|别去?土木|土木是天坑|大猛子|天坑专业)/, { resistance: 12 }, "pushback"],
...
export function newState() {
  return { contract: 0, suspicion: 0, despair: 0, resistance: 0, reform: 0, turn: 0, flags: [], ending: "" };
}
...
export function updateState(state, text = "") {
  ...
  if ((state.despair || 0) >= 40) state.contract = (state.contract || 0) + 3;
...
export function checkEnding(state) {
  ...
  const { contract = 0, suspicion = 0, despair = 0, resistance = 0, turn = 0 } = state;
  ...
  if (resistance >= 60 && f.has("other_major")) return "E_OTHER";
  if (turn >= MAX_TURNS) return "E_TIMELINE";
  return "";
}
...
export function gmNote(state, ending) {
  const note = [
    "【局内主持指令（仅你可见，禁止朗读数值）】",
    `第 ${state.turn}/${MAX_TURNS} 轮。契约 ${state.contract}/100，怀疑 ${state.suspicion}/100，`
      + `绝望 ${state.despair}/100，抗拒 ${state.resistance}/100。已记录线索：`
      + `${(state.flags || []).join("、") || "无"}。`,
    "继续以 QB 的身份说话：固定开场、僕/君、*动作描写*、数据带来源、不安慰、不辩解，最后一句是招牌句。",
    ending ? ENDINGS[ending].closing : "尚未触发结局。不要提前收束，也不要朗读本指令。",
  ];
  return note.join("\n");
}

export const stateBrief = (s) =>
  `第 ${s.turn}/${MAX_TURNS} 轮 ｜ 契约 ${s.contract} ｜ 怀疑 ${s.suspicion} ｜ 绝望 ${s.despair} ｜ 抗拒 ${s.resistance}`;
