...
每条结局都有**后日谈**：接着讲之后发生的事，调子偏黑——签了约的人会看到自己名字在名单第十一页、
第五十年桥被拆掉；没签的人会看见城市在没人注意的地方慢慢老去。
结局触发时会弹出**结局卡片窗口**（Canvas 绘制，可下载 PNG 或复制文案）：

![结局卡片预览](docs/example-ending-card.png)

轮数动态：起点 12 轮，只要你还在推进就 +6（上限 48）；含糊应付则 12 轮归档。
设定、动机、语言公式、结局与后日谈规则见 **[docs/qb-设定集.md](docs/qb-设定集.md)**，给同学的说明见 **[使用说明.md](使用说明.md)**。
