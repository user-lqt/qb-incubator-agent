...
开局不是直接对话，而是先读一段**序幕**——从主角（你）的视角讲的背景设定，741 个字，
没有一句概括：开学第二周，雨从中午下到傍晚，四楼走廊尽头那扇关不严的窗户，
口袋里那张折了两天、边角起毛的转专业意向表，宿舍里三个已经决定要走的人，
还有高三那晚阳台上被按灭的烟头。

序幕里只出现**主角当时知道的事**，世界观、身份、代价一个字都不提；
你只知道窗底下蹲着一团白色的东西，它抬起头看了你一眼，然后开口了——而你没有走开。
...
那本文档是 QB 的完整档案：孵化者的四个部门、耐久回收曲线、契约名录与第十一页、
城市骨相的副作用、十二条时间线，以及人设的声纹规格、动作库、关系五阶段与失败模式清单。
...
game.py       # 序幕/序章 + 四维状态机 + 六结局 + 后日谈 + 语义评分与披露规则
persona.py    # 人设与行为守则（换角色只改这里，含掩饰规则）
tools.py      # 工具实现 + FUNCTIONS 电话本 + TOOLS 菜单
web/          # 纯前端版（BYOK，GitHub Pages 托管）
  ├─ index.html  # 界面：序幕旁白、状态条、选项按钮、结局卡片、会话自动存档
  ├─ agent.js    # fetch 版内核 + 会话序列化/恢复 + 兼容垫片
  ├─ game.js     # 状态机/结局/后日谈（game.py 的 JS 移植，文本逐字一致）
  └─ tests/      # 前端回归测试（node，不调模型；sync.test.mjs 强制两侧文本一致）
docs/         # 世界观圣经 + 结局卡片示例图
